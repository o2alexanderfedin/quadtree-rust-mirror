...
pub(crate) extern "C" fn quadtree_bounds_new() -> *mut QuadtreeBoundsT {
    ...
    if {
                    bounds =
                        unsafe {
                                malloc(core::mem::size_of::<QuadtreeBoundsT>() as u64)
    ...
    unsafe {
        (*bounds).nw =
            quadtree_point_new(f32::INFINITY as f64, -f32::INFINITY as f64)
    };
    unsafe {
        (*bounds).se =
            quadtree_point_new(-f32::INFINITY as f64, f32::INFINITY as f64)
...
pub(crate) extern "C" fn quadtree_bounds_extend(bounds: &mut QuadtreeBoundsT,
    x: f64, y: f64) -> () {
    unsafe {
        (*(*bounds).nw).x = unsafe { fmin(x, unsafe { (*(*bounds).nw).x }) }
    };
    unsafe {
        (*(*bounds).nw).y = unsafe { fmax(y, unsafe { (*(*bounds).nw).y }) }
    };
    unsafe {
        (*(*bounds).se).x = unsafe { fmax(x, unsafe { (*(*bounds).se).x }) }
    };
    unsafe {
        (*(*bounds).se).y = unsafe { fmin(y, unsafe { (*(*bounds).se).y }) }
    };
    (*bounds).width =
        unsafe {
            fabs(unsafe { (*(*bounds).nw).x } - unsafe { (*(*bounds).se).x })
        };
    (*bounds).height =
        unsafe {
            fabs(unsafe { (*(*bounds).nw).y } - unsafe { (*(*bounds).se).y })
        };
}
