...
use crate::src::node::{
    quadtree_node_free, quadtree_node_isempty, quadtree_node_isleaf,
    quadtree_node_ispointer, quadtree_node_reset, quadtree_node_with_bounds,
};
use crate::src::point::{quadtree_point_free, quadtree_point_new};
use crate::src::quadtree_h::{
    QuadtreeNode, QuadtreeNodeT, QuadtreePointT, QuadtreeT,
};

pub(crate) extern "C" fn quadtree_new(minx: f64, miny: f64, maxx: f64,
    maxy: f64) -> *mut QuadtreeT {
    let mut tree: *mut QuadtreeT = core::ptr::null_mut();
    if ({
                            tree =
                                unsafe { malloc(core::mem::size_of::<QuadtreeT>() as u64) }
                                    as *mut QuadtreeT;
                            tree
                        }).is_null() as i32 != 0 {
...
pub(crate) extern "C" fn quadtree_free(tree: *mut QuadtreeT) -> () {
    if unsafe { (*tree).key_free.is_some() } {
        quadtree_node_free(unsafe { (*tree).root },
            unsafe { (*tree).key_free });
    } else { quadtree_node_free(unsafe { (*tree).root }, Some(elision_)); }
    unsafe { free(tree as *mut ()) };
}

extern "C" fn node_contains_(outer: &QuadtreeNodeT, it: &QuadtreePointT)
    -> i32 {
    return ((*outer).bounds as *mut () != 0 as *mut () &&
                ...
                &&
                unsafe { (*unsafe { (*(*outer).bounds).se }).y } <= (*it).y)
            as i32;
}

extern "C" fn get_quadrant_(root: &QuadtreeNodeT, point: *mut QuadtreePointT)
    -> *mut QuadtreeNodeT {
...
extern "C" fn find_(node: *mut QuadtreeNodeT, x: f64, y: f64)
    -> *mut QuadtreePointT {
...
pub(crate) extern "C" fn quadtree_search(tree: &QuadtreeT, x: f64, y: f64)
    -> *mut QuadtreePointT {
...
extern "C" fn reset_node_(tree: &QuadtreeT, node: *mut QuadtreeNodeT) -> () {
    ...
    } else { quadtree_node_reset(unsafe { &*node }, Some(elision_)); }
}

extern "C" fn split_node_(tree: *mut QuadtreeT, node: *mut QuadtreeNodeT)
    -> i32 {
    ...
    if ({
                            ne =
                                quadtree_node_with_bounds(x + hw, y - hh, x + hw * 2 as f64,
                                    y);
                            ne
                        }).is_null() as i32 != 0 {
        return 0;
    ...
    if ({
                            sw =
                                quadtree_node_with_bounds(x, y - hh * 2 as f64, x + hw,
                                    y - hh);
                            sw
                        }).is_null() as i32 != 0 {
        return 0;
    ...
    if ({
                            se =
                                quadtree_node_with_bounds(x + hw, y - hh * 2 as f64,
                                    x + hw * 2 as f64, y - hh);
                            se
                        }).is_null() as i32 != 0 {
        return 0;
...
extern "C" fn insert_(tree: *mut QuadtreeT, root: *mut QuadtreeNodeT,
    point: *mut QuadtreePointT, key: *mut ()) -> i32 {
    if quadtree_node_isempty(root) != 0 {
        ...
        if unsafe { (*unsafe { (*root).point }).x } == unsafe { (*point).x }
            ...
            if (split_node_(tree, root) == 0) as i32 != 0 { return 0; }
            return insert_(tree, root, point, key);
        }
    } else if quadtree_node_ispointer(root) != 0 {
        let quadrant: *mut QuadtreeNodeT =
            get_quadrant_(unsafe { &*root }, point);
...
pub(crate) extern "C" fn quadtree_insert(tree: *mut QuadtreeT, x: f64, y: f64,
    key: *mut ()) -> i32 {
    let mut point: *mut QuadtreePointT = core::ptr::null_mut();
    let mut insert_status: i32 = 0;
    if ({ point = quadtree_point_new(x, y); point }).is_null() as i32 != 0 {
        return 0;
    }
    if (node_contains_(unsafe { &*unsafe { (*tree).root } },
                        unsafe { &*point }) == 0) as i32 != 0 {
        quadtree_point_free(point);
        return 0;
    }
    if ({
                        insert_status =
                            insert_(tree, unsafe { (*tree).root }, point, key);
                        insert_status
                    } == 0) as i32 != 0 {
        quadtree_point_free(point);
        return 0;
...
pub(crate) extern "C" fn quadtree_walk(root: *mut QuadtreeNodeT,
    ...
    if unsafe { (*root).nw } as *mut () != 0 as *mut () {
        quadtree_walk(unsafe { (*root).nw } as *mut QuadtreeNodeT, descent,
            ascent);
    }
    if unsafe { (*root).ne } as *mut () != 0 as *mut () {
        quadtree_walk(unsafe { (*root).ne } as *mut QuadtreeNodeT, descent,
            ascent);
    }
    if unsafe { (*root).sw } as *mut () != 0 as *mut () {
        quadtree_walk(unsafe { (*root).sw } as *mut QuadtreeNodeT, descent,
            ascent);
    }
    if unsafe { (*root).se } as *mut () != 0 as *mut () {
        quadtree_walk(unsafe { (*root).se } as *mut QuadtreeNodeT, descent,
            ascent);
    }
