...
use crate::src::quadtree_h::{QuadtreeBoundsT, QuadtreeNode, QuadtreeNodeT, QuadtreePointT};
...
pub(crate) extern "C" fn quadtree_node_new() -> *mut QuadtreeNodeT {
    ...
    if ({
        node =
            unsafe { malloc(core::mem::size_of::<QuadtreeNodeT>() as u64) } as *mut QuadtreeNodeT;
        node
    })
    .is_null() as i32
        != 0
...
pub(crate) extern "C" fn quadtree_node_reset(
    node: &QuadtreeNodeT,
...
pub(crate) extern "C" fn quadtree_node_free(
    node: *mut QuadtreeNodeT,
    key_free: Option<unsafe extern "C" fn(*mut ()) -> ()>,
) -> () {
    if unsafe { (*node).nw } as *mut () != 0 as *mut () {
        quadtree_node_free(unsafe { (*node).nw } as *mut QuadtreeNodeT, key_free);
    }
    if unsafe { (*node).ne } as *mut () != 0 as *mut () {
        quadtree_node_free(unsafe { (*node).ne } as *mut QuadtreeNodeT, key_free);
    }
    if unsafe { (*node).sw } as *mut () != 0 as *mut () {
        quadtree_node_free(unsafe { (*node).sw } as *mut QuadtreeNodeT, key_free);
    }
    if unsafe { (*node).se } as *mut () != 0 as *mut () {
        quadtree_node_free(unsafe { (*node).se } as *mut QuadtreeNodeT, key_free);
    }
...
pub(crate) extern "C" fn quadtree_node_ispointer(node: *mut QuadtreeNodeT) -> i32 {
    return (unsafe { (*node).nw } as *mut () != 0 as *mut ()
        ...
        && (quadtree_node_isleaf(unsafe { &*node }) == 0) as i32 != 0) as i32;
}

pub(crate) extern "C" fn quadtree_node_isempty(node: *mut QuadtreeNodeT) -> i32 {
    return (unsafe { (*node).nw } as *mut () == 0 as *mut ()
        && unsafe { (*node).ne } as *mut () == 0 as *mut ()
        && unsafe { (*node).sw } as *mut () == 0 as *mut ()
        && unsafe { (*node).se } as *mut () == 0 as *mut ()
...
pub(crate) extern "C" fn quadtree_node_with_bounds(
    minx: f64,
    miny: f64,
    maxx: f64,
    maxy: f64,
    ...
    quadtree_bounds_extend(unsafe { &mut *unsafe { (*node).bounds } }, maxx, maxy);
    quadtree_bounds_extend(unsafe { &mut *unsafe { (*node).bounds } }, minx, miny);
    return node;
}
